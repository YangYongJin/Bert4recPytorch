def set_template(args):
    if args.template is None:
        ...
        args.dataset_code = 'ml-' + \
            input('Input 1 for ml-1m, 20 for ml-20m: ') + 'm'
        args.min_rating = 0 if args.dataset_code == 'ml-1m' else 4  # data에서 걸러낼 최소 rating
        args.min_uc = 5  # min_uc 개이상안적은  user 걸러
        args.min_sc = 0  # 아이템이 min_sc 개 이상 없으면 걸러
        args.split = 'leave_one_out'  # 한 sequence를 나누는 것 hold out이면 user를 나누는 것
        ...
        args.train_negative_sampler_code = 'random'  # popular or random
        args.train_negative_sample_size = 0
        args.train_negative_sampling_seed = 0
        args.test_negative_sampler_code = 'random'  # popular or random
        args.test_negative_sample_size = 100  # negative sample 개수
        args.test_negative_sampling_seed = 98765

        args.trainer_code = 'bert'
        args.device = 'cuda'  # train device
        args.num_gpu = 1
        args.device_idx = '0'
        args.optimizer = 'Adam'  # optimizer
        args.lr = 0.001
        ...
        args.best_metric = 'NDCG@10'  # 가장 좋은 모델 고를 때 기준
        ...
        args.bert_max_len = 30  # sequence 길이
